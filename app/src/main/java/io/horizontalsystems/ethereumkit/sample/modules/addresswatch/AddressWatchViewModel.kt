...
class AddressWatchViewModel : ViewModel() {
    ...
    private fun createKit(wordList: List<String>): EthereumKit {
        ...
        when (Configuration.chain) {
            ...
            Chain.BinanceSmartChainTest -> {
                transactionSource = TransactionSource.bscScanTest(Configuration.bscScanTestKey)
                rpcSource = RpcSource.binanceSmartTestChainHttp()
            }
            Chain.PolygonTest -> {
                transactionSource = TransactionSource.polygonScanTest(Configuration.polygonTestKey)
                rpcSource = RpcSource.polygonTestRpcHttp()
            }
